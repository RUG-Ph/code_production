...
ForecastCallVolumeRecovery <- R6Class("ForecastCallVolumeRecovery",
  private = list(
    fits = NULL,
    actuals = NULL,
    llimits = NULL,
    ...
    xlim = NULL,
    ylim = NULL,
    forecast_call_volume_recovery = function(call_volume, scenarios, model) {
      call_volume_df <- data.frame(date=as.Date(call_volume), volume=as.numeric(call_volume))
      for (i in 1:nrow(scenarios)) {
        temp0 <- private$recovery_forecast(scenario=scenarios[i,], model=model)
  ...
  public = list(
    initialize = function(call_volume, scenarios, model) {
      private$forecast_call_volume_recovery(call_volume, scenarios, model)
    },
    plot_forecast = function() {
      options(repr.plot.width=15, repr.plot.height=7.5)
      plot(private$fits, lwd=2, lty=1, col='blue', xlim=private$xlim, ylim=private$ylim, ylab='call_volume')
      lines(private$ulimits, lwd=1, lty=2, col='blue')
      lines(private$llimits, lwd=1, lty=2, col='blue')
      lines(private$actuals, lwd=2, lty=1, col='black')
    }
  )
)
